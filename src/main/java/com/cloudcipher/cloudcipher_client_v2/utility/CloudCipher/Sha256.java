...
public class Sha256 {
	...
	public int[] hash(int[] message) {
		...
		for (int i = 0; i < words.length; i++) {
				words[i] = (0xff & padded[i*4]) << 24 | (0xff & padded[i*4+1]) << 16 | (0xff & padded[i*4+2]) << 8
						| (0xff & padded[i*4+3]) << 0;
		}
		...
		int pointer = 0;
		for (int j = 0; j < 8; j++) {
			int[] a = { (H[j] >>> 24) & 0xFF, (H[j] >>> 16) & 0xFF, (H[j] >>> 8) & 0xFF,
					(H[j] >>> 0) & 0xFF };
			System.arraycopy(a, 0, result, pointer, 4);
			pointer += 4;
		}
	 ...
	 */
	public static int[] pad(int[] message) {
		final int words = 64;

		// new message length: original + 1-byte and padding + 8-byte length
		int newMessageLength = message.length + 1 + 8;
		int padBytes = words - (newMessageLength % words);
		newMessageLength += padBytes;

		// copy message to extended array
		int[] paddedMessage = new int[newMessageLength];
		...
		// write 8-byte integer describing the original message length in bits
		long msgLenBin = message.length * 8;
		byte[] bytes = ByteBuffer.allocate(Long.SIZE / Byte.SIZE).putLong(msgLenBin).array();
		paddedMessage[paddedMessage.length-2] = (bytes[0] & 0xFF) << 24 | (bytes[1] & 0xFF) << 16
				| (bytes[2] & 0xFF) << 8 | (bytes[3] & 0xFF);
		paddedMessage[paddedMessage.length-1] = (bytes[4] & 0xFF) << 24 | (bytes[5] & 0xFF) << 16
